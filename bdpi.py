...
class Critic(Learner):
    ...
    def get_train_result(self,f_Q_values):
        return self._train(self._data,f_Q_values)
    ...
    def _train(self, data,f_Q_values):
        # Unpack data
    ...
    def _train_loop(self, states, actions, rewards, critic_qvalues, next_states, next_indexes,f_Q_values):
        ...
        print(critic_qvalues.min(),critic_qvalues.mean(),critic_qvalues.max(),file=f_Q_values)
...
class BDPI(object):
    ...
    def _predict_probas(self, state,f_Q_values):
    ...
    def train(self,f_Q_values):
        ...
        for critic in critics:
            ...
            states, actions, critic_qvalues = critic.get_train_result(f_Q_values)
    ...
    def select_action(self, state, env_state, possible_actions,f_probs,f_probs_n,f_Q_values):
        ...
        if self.policy is not None:
            ...
            print(probas,file=f_probs)
        if probas is None:
            probas = self._predict_probas(state,f_Q_values)
            print(probas,file=f_probs)
            

        #Filter on possible actions 
